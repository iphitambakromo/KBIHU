...
const SKEMA = [
  ...
  /* v2.0: tabel native app & kawal rombongan — WAJIB sama dengan schema.sql (fix K1: sebelumnya hanya ada di schema.sql) */
  `CREATE TABLE IF NOT EXISTS deteksi (id INTEGER PRIMARY KEY AUTOINCREMENT, mac_tag TEXT NOT NULL, jamaah_id TEXT, device_id TEXT, lat REAL, lng REAL, rssi INTEGER, sumber TEXT DEFAULT 'native', waktu TEXT NOT NULL)`,
  `CREATE TABLE IF NOT EXISTS kawal_log (id INTEGER PRIMARY KEY AUTOINCREMENT, rombongan_id TEXT NOT NULL, ketua_device TEXT, lat REAL, lng REAL, jumlah_deteksi INTEGER DEFAULT 0, waktu TEXT NOT NULL)`,
  `CREATE TABLE IF NOT EXISTS kawal_jamaah (id INTEGER PRIMARY KEY AUTOINCREMENT, rombongan_id TEXT NOT NULL, jamaah_id TEXT NOT NULL, mac_tag TEXT, status DEFAULT 'tidak_terdeteksi', rssi INTEGER, jarak_meter REAL, terakhir_terdeteksi TEXT, lat REAL, lng REAL, waktu TEXT NOT NULL)`,
  `CREATE TABLE IF NOT EXISTS kawal_alert (id INTEGER PRIMARY KEY AUTOINCREMENT, rombongan_id TEXT NOT NULL, jamaah_id TEXT NOT NULL, tipe TEXT DEFAULT 'hilang', durasi_menit INTEGER, lat REAL, lng REAL, ditangani INTEGER DEFAULT 0, waktu TEXT NOT NULL)`,
...
export async function skema(DB) {
  ...
  /* DB remote lama pernah dibuat tanpa lat/lng di deteksi — pastikan ada sebelum INSERT memakainya */
  await pastikanKolom(DB, 'deteksi', 'lat', 'REAL');
  await pastikanKolom(DB, 'deteksi', 'lng', 'REAL');
}
